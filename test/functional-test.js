...
function testItemFindByDescription() {
    return new Promise((resolve, reject) => {
        ...
        request('GET', `/firmwares?description=Free Range`, null, (err, data) => {
            if (err) {
...
function testItemFindByUpc() {
    return new Promise((resolve, reject) => {
        const expected_firmware_description = 'Pacific Natural Foods Broth Free Range Chicken Organic - 4 Ct';
        const expected_upc = '052603054294'
        const expected_model_description = 'Pacific';
        const expected_model_manufacturer = 'Pacific Foods of Oregon, Inc.';
        request('GET', `/firmwares?upc=052603054294`, null, (err, data) => {
            if (err) {
            ...
            if (data) {
                try {
                    ...
                    assertEqual(jsonData.model_description, expected_model_description);
                    assertEqual(jsonData.manufacturer, expected_model_manufacturer);
...
function testListsAddItem() {
    return new Promise((resolve, reject) => {
        const listId = 1;
        request('POST', `/lists/${listId}/firmwares`, `{ "itemId" : 10, "quantity" : 2 }`, (err, data) => {
            if (err) {
