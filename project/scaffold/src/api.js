...
class apiScaffold extends Base {
	...
	static scaffoldSchema(type) {
		...
		module.exports = {
			create: ${type.name}Mutations.create${this.capitalize(type.name)},
			delete: ${type.name}Mutations.delete${this.capitalize(type.name)},
			update: ${type.name}Mutations.update${this.capitalize(type.name)},
			search: ${type.name}Queries.${type.name}s,
		}`;
